...
void app_main(void)
{
    ...
    //1. config adc_oneshot variables and handle
    int adc_read0;
    int mv_output;
    double lux;

    adc_oneshot_unit_handle_t handle = NULL;
    adc_oneshot_unit_init_cfg_t init_configl = {
        .unit_id = ADC_UNIT_1,
        .ulp_mode = ADC_ULP_MODE_DISABLE,
    };
    ESP_ERROR_CHECK(adc_oneshot_new_unit(&init_configl, &handle));
